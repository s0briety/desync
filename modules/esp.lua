...
local EspObject = class();

function EspObject:init(player, interface)
...
local ChamObject = class();

function ChamObject:init(player, interface)
	self.player = assert(player, "Missing argument #1 (Player expected)");
	self.interface = assert(interface, "Missing argument #2 (table expected)");
	self:Construct();
end
...
local InstanceObject = class();

function InstanceObject:init(instance, options)
...
local EspInterface = class();

function EspInterface:init()
...
function EspInterface:AddInstance(instance, options)
	local cache = self._objectCache;
	if cache[instance] then
		warn("Instance handler already exists.");
	else
		cache[instance] = { InstanceObject(instance, options) };
	end
	return cache[instance][1];
end

function EspInterface:Load()
	assert(not self._hasLoaded, "Esp has already been loaded.");

	local function createObject(player)
		self._objectCache[player] = {
			EspObject(player, self),
			ChamObject(player, self)
		};
	end
	...
	self.playerAdded = players.PlayerAdded:Connect(createObject);
	self.playerRemoving = players.PlayerRemoving:Connect(removeObject);
	self._hasLoaded = true;
end

function EspInterface:Unload()
	assert(self._hasLoaded, "Esp has not been loaded yet.");

	for index, object in next, self._objectCache do
		for i = 1, #object do
			object[i]:Destruct();
		end
		self._objectCache[index] = nil;
	end
...
function EspInterface:getHealth(player)
	local character = player and self:getCharacter(player);
